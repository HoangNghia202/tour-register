-- =============================================================================
-- FINAL DB INIT — standalone, run once on a brand-new Supabase project.
--
-- This is a hand-cleaned flatten of migrations/0001_init.sql .. 0011 into the
-- final state. It does NOT replace the migrations/ folder (kept as history) —
-- use this file to bootstrap a fresh database in one shot.
--
--   * All columns snake_case.
--   * registrations.id / companions.id are bigint identity.
--   * Pricing lives only in destination_pricing:
--       total_price = destination_pricing.price(tour.destination, route)
--                     * (1 + adult companions),   route = 'self' | pickup point
--   * Capacity consumed per registration = 1 (employee) + adult companions.
--   * Each employee: one registration + one "đăng ký lại" (resubmit) allowance.
--
-- Run against an EMPTY project. The table/seed statements are idempotent, but
-- `create function` / `create policy` will error if the objects already exist —
...
--   employees / registrations / companions: no anon policy -> every direct
--   client access is denied; clients only reach them through the exact-match
--   SECURITY DEFINER functions below. Bulk listing/export is service-role only,
--   via /api/admin/*.
-- ---------------------------------------------------------------------------
...
create function public.find_employee_by_id(p_id text)
returns table (
  id          text,
  full_name   text,
  store_id    text,
...
create function public.get_registration_by_employee(p_employee_id text)
returns table (
  id               bigint,
  employee_id      text,
  tour_id          text,
...
-- ---------------------------------------------------------------------------
-- submit_registration — capacity check + route pricing + inserts, one tx.
--   Companion caps: <= 4 adults, <= 2 children.
--   p_total_price is accepted so the /api/register RPC call has a stable arg
--   list, but the price is always recomputed server-side.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- resubmit_registration — one-time "đăng ký lại" per employee.
--   Deletes the previous registration + companions, gives its capacity back to
--   tours.registered_count, then delegates to submit_registration() for the
--   normal capacity / pricing / insert path and stamps resubmit_count = 1.
--   One transaction: a TOUR_FULL (or any) failure rolls the delete back, so the
--   employee keeps the original registration.
-- ---------------------------------------------------------------------------

create function public.resubmit_registration(
  p_employee_id text,
  p_tour_id text,
  p_transport_method text,
  p_pickup_point text,
...
declare
  ...
  v_result          jsonb;
begin
  select id, tour_id, coalesce(resubmit_count, 0)
